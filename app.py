...
ALGORITHMS = {
    "Proposed: PQ-GAPR": {
        ...
        "msg_cycles": 6_800,
        "epoch_cycles": 12_400,
        "handshake_tx_bytes": 32,
        "handshake_rx_bytes": 48,
        "msg_tx_overhead_bytes": 30,
        "epoch_rx_bytes": 48,
        "stack_ram_bytes": 384,
        "flash_bytes": 5_820,
    ...
    "Direct PQC: ML-KEM-512 (FIPS 203)": {
        ...
        "msg_cycles": 2_100,
        "epoch_cycles": 0,
        "handshake_tx_bytes": 768,
        "handshake_rx_bytes": 800,
    ...
    "Direct PQC: ML-KEM-768 (FIPS 203)": {
        ...
        "msg_cycles": 2_100,
        "epoch_cycles": 0,
        "handshake_tx_bytes": 1088,
        "handshake_rx_bytes": 1184,
        "msg_tx_overhead_bytes": 28,
        "epoch_rx_bytes": 0,
    ...
    "Classical: ECDHE (secp256r1)": {
        ...
        "msg_cycles": 1_850,
        "epoch_cycles": 0,
        "handshake_tx_bytes": 64,
        "handshake_rx_bytes": 64,
        "msg_tx_overhead_bytes": 28,
        "epoch_rx_bytes": 0,
...
with st.sidebar:
    ...
    reporting_interval_s = st.select_slider(
        ...
        options=[10, 30, 60, 300, 600, 1800, 3600, 86400],
        value=300,
...
def calculate_simulation(payload_b, n_sess, epoch_n, interval_s):
    ...
    tx_pwr_w = NET_TX_PWR_MW / 1000.0
    rx_pwr_w = NET_RX_PWR_MW / 1000.0
    
    for name, algo in ALGORITHMS.items():
        is_gapr = "PQ-GAPR" in name
        is_hs = algo["handshake_cycles"] > 0
        
        # Computation latencies (seconds)
        ...
        # Packet fragmentation over LoRaWAN (38 bytes usable MTU)
        ...
        hs_pkts = int(np.ceil(hs_tx_tot / USABLE_MTU)) if hs_tx_tot > 0 else 0
        hs_tx_bytes = (hs_pkts * NET_HEADER_BYTES) + hs_tx_tot if hs_tx_tot > 0 else 0
        
        # Airtime (seconds)
        air_msg_s = (msg_tx_bytes * NET_AIRTIME_MS_PER_BYTE) / 1000.0
        air_hs_s = (hs_tx_bytes * NET_AIRTIME_MS_PER_BYTE) / 1000.0
        air_epoch_s = (algo["epoch_rx_bytes"] * NET_AIRTIME_MS_PER_BYTE) / 1000.0
        
        # Energy per event (Joules)
        e_msg_compute_j = mcu_pwr_w * t_msg_s
        e_msg_radio_j = tx_pwr_w * air_msg_s
        e_msg_j = e_msg_compute_j + e_msg_radio_j
        
        e_hs_compute_j = mcu_pwr_w * t_hs_s
        e_hs_radio_j = tx_pwr_w * air_hs_s
        e_hs_j = e_hs_compute_j + e_hs_radio_j
        
        e_epoch_compute_j = mcu_pwr_w * t_epoch_s
        e_epoch_radio_j = rx_pwr_w * air_epoch_s
        e_epoch_j = e_epoch_compute_j + e_epoch_radio_j
        
        # Cumulative session loop
        cum_e_mj = 0.0
        cum_compute_mj = 0.0
        cum_radio_mj = 0.0
        cum_tx_b = 0
        ...
        for s in range(1, n_sess + 1):
            ...
            if is_gapr and (s % epoch_n == 0):
                sess_e += e_epoch_j
                sess_comp += e_epoch_compute_j
                sess_radio += e_epoch_radio_j
            elif ("ML-KEM" in name or "ECDHE" in name) and (s % epoch_n == 0):
                sess_e += e_hs_j
                sess_comp += e_hs_compute_j
                sess_radio += e_hs_radio_j
                sess_tx += hs_tx_bytes
        ...
        avg_active_e_j = (cum_e_mj / n_sess) / 1000.0
        t_active_s = t_msg_s + air_msg_s
        t_sleep_s = max(0.0, interval_s - t_active_s)
        avg_sleep_e_j = sleep_pwr_w * t_sleep_s
        avg_pwr_w = (avg_active_e_j + avg_sleep_e_j) / interval_s
        
        self_dis_w = BAT_TOTAL_JOULES * (BAT_SELF_DISCHARGE_YR / (365.25 * 86400))
        tot_power_w = avg_pwr_w + self_dis_w
        bat_lifetime_years = (BAT_TOTAL_JOULES / tot_power_w) / (86400 * 365.25)
...
# ==============================================================================
# 6. BENCHMARK GRAPHS (HIGH-CLARITY GROUPED & COMPONENT VISUALS)
# ==============================================================================
...
# Chart 3: NEW CLEAR VISUAL - Energy Progression at Session Checkpoints (Grouped Bar Chart)
with row2_col1:
    # Sample checkpoints: 25%, 50%, 75%, 100% of workload
    checkpoints = [
    ...
    checkpoints = sorted(list(set(checkpoints))) # remove duplicates
    ...
    fig_cp = px.bar(
        ...
        text="Label",
        title="3. Energy Consumption at Session Milestones (Grouped Bar Comparison)"
    )
...
# Chart 4: Handshake Packet Fragmentation
with row2_col2:
    chart_frag_data = []
    for k, v in sim.items():
        pkts = v["hs_packets"]
        label_str = f"{pkts} Frame(s)" if pkts > 0 else "0 (No Handshake)"
        chart_frag_data.append({
            "Algorithm": k,
            "Handshake Network Frames": pkts,
            "Label": label_str
        })
    df_frag = pd.DataFrame(chart_frag_data)
    
    fig_frag = px.bar(
        df_frag, x="Algorithm", y="Handshake Network Frames", color="Algorithm",
        color_discrete_map=COLOR_MAP, text="Label",
        title="4. Handshake Packet Fragmentation (LoRaWAN DR0 / 51B MTU)"
    )
    fig_frag.update_traces(textposition='outside', textfont_size=11)
    fig_frag.update_layout(
        showlegend=False, 
        xaxis_tickangle=-22,
        yaxis=dict(range=[0, max(df_frag["Handshake Network Frames"]) * 1.22]),
        margin=dict(t=50, b=80)
    )
...
for k, v in sim.items():
    ...
    table_rows.append({
        ...
        "Handshake Frames": f"{v['hs_packets']} frame(s)",
        "Msg Latency (ms)": f"{v['msg_latency_ms']:.3f} ms",
...
# ==============================================================================
# 8. NEW CLEAR VISUAL: DUTY-CYCLE LIFETIME GROUPED BAR COMPARISON
# ==============================================================================
st.markdown("---")
st.subheader("🔋 Battery Operating Lifetime Across Standard IoT Duty Cycles")
st.markdown("Comparing operating battery lifespan (Years) on a **CR2032 Coin Cell (225 mAh)** across 4 standard IoT duty cycles:")

duty_cycles = [
    {"label": "⚡ High-Rate (Every 30s)", "interval_s": 30},
    {"label": "⏱️ Active Sensor (Every 5 mins)", "interval_s": 300},
    {"label": "🌾 Periodic Meter (Every 1 Hour)", "interval_s": 3600},
...
# Optional view toggle for continuous curve
with st.expander("📈 View Continuous Log-Scale Duty Cycle Curve"):
    intervals_sweep = [10, 30, 60, 300, 600, 1800, 3600, 86400]
    fig_life = go.Figure()
...
\\begin{tabular}{lcccc}
\\hline
\\textbf{Algorithm} & \\textbf{RAM (B)} & \\textbf{Flash (KB)} & \\textbf{Frames} & \\textbf{Energy (mJ)} \\\\
\\hline
"""
    for k, v in sim.items():
        latex_table += f"{k} & {v['stack_ram_bytes']} & {v['flash_bytes']/1024:.1f} & {v['hs_packets']} & {v['total_energy_mj']:.1f} \\\\\n"
    latex_table += """\\hline
\\end{tabular}
\\end{table}
"""
